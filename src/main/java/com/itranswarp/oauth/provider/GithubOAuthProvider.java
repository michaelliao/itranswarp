...
public class GithubOAuthProvider extends AbstractOAuthProvider {
	...
	public String getAuthenticateUrl(String redirectUrl) {
		...
		return String.format(
				"https://github.com/login/oauth/authorize?client_id=%s&response_type=%s&state=%s&redirect_uri=%s",
				this.configuration.getClientId(), "code", state,
	...
	public OAuthAuthentication getAuthentication(String code, String state, String redirectUrl) throws Exception {
		...
		HttpRequest requestAccessToken = HttpRequest.newBuilder()
				.uri(new URI("https://github.com/login/oauth/access_token")).timeout(DEFAULT_TIMEOUT)
		...
		// get user info:
		HttpRequest requestUser = HttpRequest.newBuilder().uri(new URI("https://api.github.com/user"))
				.timeout(DEFAULT_TIMEOUT).header("Content-Type", "application/json")
				.header("Authorization", "token " + accessToken).GET().build();
