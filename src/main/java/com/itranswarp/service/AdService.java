...
@Component
public class AdService extends AbstractService<AdSlot> {
	...
	public void deleteAdSlot(long id) {
		...
		if (period != null) {
			throw new ApiException(ApiError.OPERATION_FAILED, "adSlot",
					"Could not delete adslot with active adperiod.");
		}
		this.db.remove(slot);
	}
	...
	public AdMaterial createAdMaterial(User user, AdPeriod ap, AdMaterialBean bean) {
		...
		if (isExpiredAdPeriod(ap)) {
			throw new ApiException(ApiError.OPERATION_FAILED, "AdPeriod",
					"Could not create AdMaterial for expired AdPeriod.");
		}
		bean.validate(true);
	...
	private List<AdMaterial> getActiveAdMaterialsByAdPeriod(AdPeriod period, String today) {
		return this.db.from(AdMaterial.class)
				.where("adPeriodId = ? AND startAt <= ? AND endAt > ?", period.id, today, today).orderBy("endAt").desc()
				.list();
	}
